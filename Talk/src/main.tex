...
%        $.
    ...
    \begin{frame}{Описание алгоритма MSSA}
        ...
        \begin{enumerate}
            \item \bluetext{Вложение} $\tX \mapsto \bfH = [\bbH(\tX_1):\bbH(\tX_2): \dots: \bbH(\tX_P)]\in \bbR^{L\times KP},$
            ...
            \item \bluetext{Группировка} $\tilde{\bfS}= \sum_{i=1}^{R} \sqrt{\lambda_i}U_i V_i^\rmT,\,\,
            R \leqslant d$
    ...
    \begin{frame}{Численные сравнения}
        ...
        \vspace{0.2cm}
        Точность сравнивалась по RMSE по 1000 реализациям шума $\varepsilon_n^{(p)}$,
